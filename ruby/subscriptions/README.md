...
Ruby 3.2+, a sandbox `PAGOU_API_TOKEN`, and a `pgct_*` card token from the
...
```bash
PAGOU_CARD_TOKEN=pgct_... ruby subscriptions/lifecycle.rb
```
...
`bundle exec rake test` covers the shared client and reconciliation used here. See
[`../tests`](../tests).
