...
.NET SDK 8.0+, a sandbox `PAGOU_API_TOKEN`, and a `pgct_*` card token from the
...
```bash
PAGOU_CARD_TOKEN=pgct_... dotnet run --project subscriptions
```
...
`dotnet test` covers the shared client and reconciliation used here. See [`../tests`](../tests).
