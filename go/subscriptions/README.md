...
Go 1.21+, a sandbox `PAGOU_API_TOKEN`, and a `pgct_*` card token from the
...
```bash
PAGOU_CARD_TOKEN=pgct_... go run ./subscriptions/lifecycle
```
...
2. **Subscription** — `POST /v2/subscriptions`:
   ...
   Sent with an `Idempotency-Key` so a retry reuses the same subscription.
...
`go test ./...` covers the shared client and reconciliation used here. See [`../tests`](../tests).
