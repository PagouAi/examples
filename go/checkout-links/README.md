...
The response is `{ "data": { "url": "https://..." } }`. **The `data.url` is the public identifier** —
there is no short code or UUID. Persist that URL; it is the only handle to the link.
...
- **`400`/`422`** — missing/invalid product fields; inspect the error details.
