...
Java 17+, Maven 3.9+, and a sandbox `PAGOU_API_TOKEN` (reconciliation calls the API). Configure
...
```bash
mvn -q compile exec:java@webhooks-server   # POST envelopes to http://localhost:4000/webhooks/pagou
```
...
curl -sS -X POST http://localhost:4000/webhooks/pagou \
  ...
  --data @src/test/resources/fixtures/webhook.transaction.json
```
...
- **Require the event id** — a body without a top-level `id` is answered `400 { "error":
  "missing_event_id" }` (`WebhookHandlers` → `WebhookServer`).
...
- **Respond 2xx fast** — the ack is sent before any API call; the reconciliation runs on a background
  executor (`WebhookServer`).
- **Offload slow work** — `WebhookProcessor.processEvent` does the reconciliation off the response path.
- **State change only on confirmed** — `WebhookHandlers.isConfirmedStateChange` gates which events
...
`mvn test` covers envelope routing for all three families, the missing-id rejection, dedupe, the
confirmed-vs-informational gate, and that `processEvent` reconciles and updates state only on a
confirmed event. See [`../src/test/java/ai/pagou/examples/WebhooksTest.java`](../src/test/java/ai/pagou/examples/WebhooksTest.java).
