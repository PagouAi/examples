...
Java 17+, Maven 3.9+, a sandbox `PAGOU_API_TOKEN`, and a `pgct_*` card token from the
[Payment Element](../src/main/resources/card-element/index.html) (subscription mode). Provide it as
`PAGOU_CARD_TOKEN`. **Sandbox dependency:** creates a real sandbox customer and subscription.

## Run command

```bash
PAGOU_CARD_TOKEN=pgct_... mvn -q compile exec:java@subs-demo   # subscriptions/SubscriptionLifecycle.java
```
...
`mvn test` covers the shared client and reconciliation used here. See [`../src/test`](../src/test).
