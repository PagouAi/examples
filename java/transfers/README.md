...
```bash
mvn -q compile exec:java@transfers-demo   # transfers/TransferLifecycle.java
```
...
`mvn test` covers the shared client (retry, idempotency, error mapping). See [`../src/test`](../src/test).
