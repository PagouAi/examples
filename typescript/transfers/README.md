...
```bash
npm run transfers:demo       # transfers/raw/lifecycle.ts
npm run transfers:demo:sdk   # transfers/sdk/lifecycle.ts
```
...
1. **Create** — `POST /v2/transfers`:
   ...
   { "pix_key_type": "EMAIL", "pix_key_value": "supplier@example.com",
     "amount": 5000, "description": "Supplier payout", "external_ref": "payout_1700000000000" }
   ```
...
`npm test` covers the shared client (retry, idempotency, error mapping). See [`../tests`](../tests).
