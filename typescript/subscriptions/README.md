...
Node 18.18+, a sandbox `PAGOU_API_TOKEN`, and a `pgct_*` card token from the
...
```bash
PAGOU_CARD_TOKEN=pgct_... npm run subs:demo       # subscriptions/raw/lifecycle.ts
PAGOU_CARD_TOKEN=pgct_... npm run subs:demo:sdk   # subscriptions/sdk/lifecycle.ts
```
...
1. **Customer** — `POST /v2/customers`. Customers are not an SDK resource, so both variants create
   the customer through the raw client. The response `data.id` (UUID) is the public customer id.
...
`npm test` covers the shared client and reconciliation used here. See [`../tests`](../tests).
